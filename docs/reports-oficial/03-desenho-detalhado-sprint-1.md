...
| Campo | Valor |
| --- | --- |
| Documento | Desenho detalhado - Sprint 1 - Identidade, Segurança e Garagem |
...
A Sprint 1 teve como objetivo: Login local, MFA, sessão, garagem base e histórico inicial. O desenho detalhado apresenta requisitos implementados, componentes técnicos, processo BPMN, interface, testes, manual de utilização e manual técnico.

## 2. Introdução

Este documento detalha apenas o incremento da Sprint 1, seguindo a separação entre análise e desenho indicada nos slides: a análise define o que o sistema deverá fazer; o desenho detalhado descreve como a solução foi organizada para implementar esses requisitos.
...
| --- | --- |
| M01, M02 | Login local, MFA, sessão, garagem base e histórico inicial |

#### 3.2.1 Requisitos funcionais implementados

| ID | Módulo | Prioridade | Estado | Requisito |
| --- | --- | --- | --- | --- |
| RF-M01-01 | M01 | Must | Implementado | O sistema deverá permitir criar conta local com nome, email e palavra-passe. |
| RF-M01-02 | M01 | Must | Implementado | O sistema deverá autenticar utilizadores locais com hash seguro de palavra-passe. |
| RF-M01-03 | M01 | Must | Implementado | O sistema deverá permitir iniciar sessão e terminar sessão de forma segura. |
| RF-M01-05 | M01 | Should | Implementado | O sistema deverá permitir ativar MFA TOTP por aplicação Authenticator compatível. |
| RF-M02-01 | M02 | Must | Implementado | O sistema deverá permitir criar, editar, consultar e remover veículos da garagem. |
| RF-M02-02 | M02 | Must | Implementado | O sistema deverá associar marca, modelo, ano, combustível, consumo e matrícula a cada veículo. |
| RF-M02-03 | M02 | Must | Implementado | O sistema deverá registar o nível atual de combustível ou carga de cada veículo. |
...
![Figura 2 - Processo BPMN da Sprint 1](assets/diagrams/bpmn-02-garagem-veiculo.png)

*Figura 2 - Processo BPMN da Sprint 1.*
...
| Campo | Valor |
| --- | --- |
| Nome Caso de teste | Validar regras principais da Sprint 1 |
| Código | UT-S1-001 |
...
| --- | --- | --- |
| IT-S1-001 | Controller + Service + EF Core | Pedido válido altera o estado persistido sem erro |

### 3.3.4 Testes de Sistema

| Código | Cenário E2E | Resultado esperado |
| --- | --- | --- |
| ST-S1-001 | Utilizador percorre o fluxo principal da Sprint 1 | Fluxo conclui sem erro bloqueante |

### 3.3.5 Testes de aceitação

| Código | Given | When | Then |
| --- | --- | --- | --- |
| UAT-S1-001 | O utilizador está autenticado | Executa o fluxo principal da sprint | O sistema apresenta resultado claro e persistente |
